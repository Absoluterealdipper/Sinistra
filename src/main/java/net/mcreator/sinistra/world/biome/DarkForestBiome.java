...
import net.mcreator.sinistra.block.DarkOAKlogBlock;
...
public class DarkForestBiome extends SinistraModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(DarkOAKlogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(Blocks.OAK_LEAVES.getDefaultState()))).baseHeight(8)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == DarkGrassBlockBlock.block.getDefaultState().getBlock()
							|| ground == DarkDirtBlock.block.getDefaultState().getBlock())
							&& (ground2 == DarkGrassBlockBlock.block.getDefaultState().getBlock()
									|| ground2 == DarkDirtBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), DarkDirtBlock.block.getDefaultState(), bbox);
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, DarkOAKlogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == DarkOAKlogBlock.block.getDefaultState().getBlock()
					|| blockType == Blocks.OAK_LEAVES.getDefaultState().getBlock()
					|| blockType == DarkGrassBlockBlock.block.getDefaultState().getBlock()
					|| blockType == DarkDirtBlock.block.getDefaultState().getBlock();
		}
