...
package net.mcreator.sinistra.world.dimension;
...
import net.mcreator.sinistra.item.SinistraDimensionItem;
...
public class SinistraDimensionDimension extends SinistraModElements.ModElement {
	@ObjectHolder("sinistra:sinistra_dimension")
	public static final ModDimension dimension = null;
	@ObjectHolder("sinistra:sinistra_dimension_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public SinistraDimensionDimension(SinistraModElements instance) {
		super(instance, 6);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("sinistra_dimension"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("sinistra:sinistra_dimension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("sinistra:sinistra_dimension"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("sinistra:sinistra_dimension"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("sinistra:dark_forest")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new SinistraDimensionItem().setRegistryName("sinistra_dimension"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("sinistra_dimension_portal");
		}
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = SinistraDimensionDimension.CustomPortalBlock.createPatternHelper(entity.world, new BlockPos(pos));
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == DarkStoneBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == DarkStoneBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == DarkStoneBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "sinistra_dimension_portal",
	...
	public static class TeleporterDimensionMod extends Teleporter {
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag ? DarkStoneBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, DarkStoneBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = SinistraDimensionDimension.CustomPortalBlock.createPatternHelper(this.world,
						blockpos);
	...
	public static class CustomDimension extends Dimension {
		...
		public Vec3d getFogColor(float cangle, float ticks) {
			return new Vec3d(0.047058823529, 0.047058823529, 0.047058823529);
		}
